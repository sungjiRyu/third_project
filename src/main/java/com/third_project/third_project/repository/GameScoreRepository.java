...
//    GameScoreEntity findByMemberRankingView(MemberRankingView mrentity);
    ...
    +"(select * from (select *, dense_rank() over(partition by b.et_time_type ,gs_et_seq order by calculated asc) as ranking from "
    +"(select *, (case when b.et_time_type = 1 then (TIME('00:00:00') + a.gs_time) when b.et_time_type = 2 then (TIME('00:00:00') - a.gs_time) end) "
            +"as calculated from game_score a join ex_type b on a.gs_et_seq = b.et_seq) as c where week(gs_reg_dt) = week(now())-1 and et_seq = :seq order by ranking) d join member_info e on d.gs_mi_seq = e.mi_seq "
    +"join member_img g on e.mi_mimg_seq = g.mimg_seq) f  ", nativeQuery = true)
